...
function buildASigninForm() {
    container.innerHTML += '<section id="signin-form-container"><form action="signin.php"><h2>Log in</h2><input type="email" name="email" id="email" placeholder="Email" required><input type="password" name="password" id="password" placeholder="Password" required><button type="submit" class="block"><a href="#posts">Login</a></button></form></section>';
